...
def exec_detect(target_image_path):
    ...
    current_app.logger.info(f"ラベル: {output_labels}")
    # 学習済みモデルが検知した各物体の分だけ画像に追記
    for box, label, score in zip(output["boxes"], output["labels"], output["scores"]):
        current_app.logger.info(f"スコア: {score}")
    ...
    current_app.logger.info(f"フォルダを作成しました: {detected_image_file_path}")
    # 変換後の画像ファイルを保存先へコピーする
    output_image = Image.fromarray(result_image)
    output_image.save(detected_image_file_path)
